...
public:
    ...
    void switchDataNodes(t_Node<T> *nodeToDelete, t_Node<T> *nodeToSwitch);
    ...
    void createFullTree(int size);
    static int FullTreeHeight (int size);
    static int numTreeLeaves (int size);
    void createFullTreeFromNode (t_Node<T>* head,int height);
...
t_Node<T>* AVLTree<T>::treeInsert(T* a) {
    ...
    if (this->m_root == nullptr) {
        m_root = b;
        listNode = m_list.listInsertInFront(a, nullptr);
        b->m_place_in_List=listNode;
        listNode->m_place_in_tree=b;
        return b;
    }
    ...
    while (iterator != nullptr) {
        if (*iterator->m_data > *a) {
            if (iterator->m_left_son == nullptr) {
                ...
                afterTreeInsert(b);
                listNode = m_list.listInsertInFront(a, iterator->m_place_in_List);
                b->m_place_in_List=listNode;
                listNode->m_place_in_tree=b;
                return b;
            }
            ...
            if (iterator->m_right_son == nullptr) {
                ...
                afterTreeInsert(b);
                listNode = m_list.listInsertAfter(a, iterator->m_place_in_List);
                b->m_place_in_List=listNode;
                listNode->m_place_in_tree=b;
                return b;
            }
...
bool AVLTree<T>::treeDelete(t_Node<T>* nodeToDelete) {
    ...
    if (nodeToDelete->m_left_son != nullptr) {
        ...
        if (iterator->m_left_son == nullptr) {
            ...
            m_list.listRemove(nodeToDelete->m_place_in_List);
            delete nodeToDelete;
            afterTreeDelete(fatherOfNodeToDelete->m_right_son);
            return true;
        }
    }
    else {
...
template<class T>
void AVLTree<T>::switchDataNodes(t_Node<T> *nodeToDelete, t_Node<T> *nodeToSwitch) { ////////////////לשנות את הסדר ברשימה בהתאם לשינוי האויבבקטים בין שני nodes
...
template<class T>
void AVLTree<T>::createFullTreeFromNode (t_Node<T>* head,int height){
    if (height==0){
        return;
    }
    if (m_root == nullptr){
        m_root=new t_Node<T>;
        head=m_root;
        createFullTreeFromNode(head,height-1);
        return;
    }
    t_Node<T>* leftSon = new t_Node<T>;
    t_Node<T>* rightSon = new t_Node<T>;
    makeLeftSonAndFather(leftSon,head);
    makeRightSonAndFather(rightSon,head);
    createFullTreeFromNode(head->m_left_son,height-1);
    createFullTreeFromNode(head->m_right_son,height-1);
}
...
template<class T>
int AVLTree<T>::FullTreeHeight (int size){
    int k=1;
    int height=1;
    while (size>k){
        k=2*(k+1)-1;
...
template<class T>
int AVLTree<T>::numTreeLeaves (int size){
    int k=1;
    int height=1;
    while (size>k){
        k=2*(k+1)-1;
    ...
    int numOfAllLeaves=(k+1)/2;
    int numOfLeftOverLeaves=2*numOfAllLeaves-1-size;
    int numOfWantedLeaves=numOfAllLeaves-numOfLeftOverLeaves;
    return numOfWantedLeaves;
}

template<class T>
void AVLTree<T>::addInorderList (t_Node<T>* treeIterator) {

    if (treeIterator == nullptr) {
        return;
    }
    addInorderList(treeIterator->m_left_son);
    if (treeIterator->isLeaf()) {
        if (this->m_list.m_good_leaves == 0) {
            if (treeIterator->m_father != nullptr) {
                fatherAndSonSeparation(treeIterator);
                updateHeightAndBF(treeIterator);
            }
            delete (treeIterator);
        }
        else {
            this->m_list.m_good_leaves--;
            treeIterator->m_data = this->m_list.m_iterator->m_data;
            treeIterator->m_place_other_tree=this->m_list.m_iterator->m_place_in_tree;
            treeIterator->m_place_in_List=this->m_list.m_iterator;
            this->m_list.m_iterator->m_place_in_tree=treeIterator;

            this->m_list.m_iterator = this->m_list.m_iterator->m_next;
            updateHeightAndBF(treeIterator);
        }
        return;
    }
    ...
    addInorderList(treeIterator->m_right_son);
    updateHeightAndBF(treeIterator);
}
...
void AVLTree<T>::deletePostOrder (t_Node<T>* treeIterator, bool toDeleteDate) {
    ...
    l_Node<T>* listIterator=treeIterator->m_place_in_List;
    if (!toDeleteDate){
    ...
    treeIterator->m_place_other_tree=nullptr;
    treeIterator->m_place_in_List=nullptr;
    listIterator->m_prev=nullptr;
    listIterator->m_next=nullptr;
    listIterator->m_place_in_tree=nullptr;
    delete treeIterator;
    delete listIterator;
}
